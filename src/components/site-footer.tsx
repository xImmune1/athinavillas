  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
        <div className="md:col-span-2">
          ...
          <div className="mt-6 flex gap-4 text-xs uppercase tracking-[0.25em]">
            <Link to="/rooms/studio" className="opacity-70 hover:opacity-100 hover:text-accent">{t("room.studio.name")}</Link>
            <Link to="/rooms/garden-view" className="opacity-70 hover:opacity-100 hover:text-accent">{t("room.garden.short")}</Link>
            <Link to="/rooms/sea-view" className="opacity-70 hover:opacity-100 hover:text-accent">{t("room.sea.short")}</Link>
          </div>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-accent mb-4">{t("footer.contact")}</p>
          <p className="text-sm opacity-80">athina-villas@hotmail.com</p>
          <p className="text-sm opacity-80 mt-1">+30 28410 41342</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-accent mb-4">{t("footer.visit")}</p>
          <Link to="/location" className="block text-sm opacity-80 hover:opacity-100">{t("nav.location")}</Link>
          <a href="https://spinalonga.book-onlinenow.net/" target="_blank" rel="noopener noreferrer" className="block text-sm opacity-80 mt-1 hover:opacity-100">{t("footer.book")}</a>
          <a
      ...
      <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-[oklch(0.97_0.012_85/0.1)] flex flex-col md:flex-row justify-between text-xs opacity-60">
        <p>© {new Date().getFullYear()} Athina Villas. {t("footer.rights")}</p>
